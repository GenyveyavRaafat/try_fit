...
def main():
    ...
    unet_path="/content/drive/MyDrive/new_checkpoints/unet/unet.pth"
    unet_dict=torch.load(unet_path,map_location=torch.device("cuda"))
    ...
    emasc_path="/content/drive/MyDrive/new_checkpoints/emasc/emasc.pth"
    emasc_dict=torch.load(emasc_path,map_location=torch.device("cuda"))
    emasc.load_state_dict(emasc_dict)
    
    inversion_adapter = torch.hub.load(repo_or_dir='miccunifi/ladi-vton', source='github', model='inversion_adapter',
                                       dataset=args.dataset)
    inversion_adapter_path="/content/drive/MyDrive/new_checkpoints/inversion_adapter/inversion_adapter.pth"
    inversion_adapter_dict=torch.load(inversion_adapter_path,map_location=torch.device("cuda"))
    inversion_adapter.load_state_dict(inversion_adapter_dict)
   
    tps, refinement = torch.hub.load(repo_or_dir='miccunifi/ladi-vton', source='github', model='warping_module',
                                     dataset=args.dataset)
    # Load the full checkpoint
    warp_ckpt = torch.load("/content/drive/MyDrive/new_checkpoints/warp/warp.pth", map_location=torch.device("cuda"))

    # Extract and load TPS + Refinement separately
    tps.load_state_dict(warp_ckpt["tps"])
    refinement.load_state_dict(warp_ckpt["refinement"])
